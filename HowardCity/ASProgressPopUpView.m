...
- (void)setup
{
    ...
    self.popUpViewCornerRadius = 1.0;
...
- (void)updatePopUpView
{
    ...
    progressString = [self.dataSource progressView:self stringForProgress:self.progress] ?: [[NSString alloc]initWithFormat:@"SPI:%@",[_numberFormatter stringFromNumber:@(self.progress * 2)]];
...
- (void)calculatePopUpViewSize
{
    ...
    for (int i=0; i<=100; i++) {
        NSString *string = [self.dataSource progressView:self stringForProgress:i];
        if (string) {
